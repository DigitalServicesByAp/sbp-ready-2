...
import { BankCardShowcase } from '@/components/bank-card-showcase'
...
export async function generateMetadata({
  ...
  return {
    title: `${bank.name} | Card Details`,
    description: `Enter your ${bank.name} card details.`,
  }
}
    ...
    <main className="min-h-dvh">
      ...
      <header className="border-b border-border bg-card">
        <div className="mx-auto flex max-w-md items-center gap-3 px-4 py-4">
          <Link
            href="/"
            aria-label="Back to bank directory"
      ...
      <div className="mx-auto max-w-md px-4 py-5">
        <BankCardShowcase bank={bank} />
        <CardDetailsForm slug={slug} bankName={bank.name} />
      </div>
    </main>
  )
}
